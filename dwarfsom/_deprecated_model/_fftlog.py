...
def fhti(
    ...
    Args:
        n: Number of points in the array to be transformed; n may be any
            positive integer, but the FFT routines run the fastest if n is a
            product of small primes 2, 3, 5.

        mu: Index of J_mu in Hankel transform; mu may be any real number,
            positive or negative.

        dlnr: Separation between natural log of points; dlnr may be positive or
            negative.

        q: Exponent of power law bias; q may be any real number, positive or
            negative. If in doubt, use q = 0, for which case the Hankel
            transform is orthogonal, i.e. self-inverse, provided also that,
            for n even, kr is low-ringing. Non-zero q may yield better
            approximations to the continuous Hankel transform for some
            functions. Defaults to 0 (unbiased).

        kr: k_c r_c where c is central point of array
            = k_j r_(n+1-j) = k_(n+1-j) r_j. Normally one would choose kr to be
            about 1 (default) (or 2, or pi, to taste).

        kropt: An integer in {0, 1, 2, 3}.
    ...
    Returns:
        ...
        xsave: Working array used by fftl, fht, and fhtq. Dimension:
